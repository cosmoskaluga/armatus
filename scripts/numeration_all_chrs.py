...
for f2L, f2R, f3L, f3R, fX in zip(glob.glob('data/2L/*.txt'), glob.glob('data/2R/*.txt'), glob.glob('data/3L/*.txt'), glob.glob('data/3R/*.txt'), glob.glob('data/X/*.txt')):
    ...
    export_csv = df.to_csv('output/'+f2L[8:-4]+'.dist', index = None, header=False)
